package penakelex.bicycleCharts.grafics.Charts.Parser.Function;


import penakelex.bicycleCharts.grafics.Charts.Parser.Exception.CalculatorException;
import penakelex.bicycleCharts.grafics.Charts.Parser.ParserManager;
		...
		for (int i = 0; i < f_x.length(); i++) {
			...
			if (character >= '0' && character <= '9') {
				...
				if (hasNumber) {
					final double numb = Double.parseDouble(number.toString());
					final String new_f_x = nextFunction(f_x.substring(i + 1));
					value = StrictMath.pow((double) numb, eval(new_f_x, xi));
					i += new_f_x.length();
					hasNumber = false;
					number = new StringBuilder();
				} else if (hasFunction) {
